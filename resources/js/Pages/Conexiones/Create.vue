...
import Modal from '@/Components/Modal.vue'; // Para el modal de "Nueva Zona"
import axios from 'axios';

const props = defineProps({
    zonas: Array,
    searchAfiliadosUrl: String, // URL de la API
    errors: Object,
});
const page = usePage();
                    ...
                    <form @submit.prevent="submit" class="space-y-6">
                        ...
                        <div class="p-4 border dark:border-gray-700 rounded-md bg-gray-50 dark:bg-gray-900">
                            ...
                            <div class="flex gap-2">
                                <input v-model="searchCi" @keydown.enter.prevent="buscarAfiliado" type="text" placeholder="Ingrese CI..." class="flex-grow border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 rounded-md shadow-sm" />
                            ...
                            <div v-if="busquedaMensaje" :class="{'text-green-600': form.afiliado_id, 'text-red-600': !form.afiliado_id}" class="text-sm mt-1">{{ busquedaMensaje }}</div>
                            <div v-if="form.errors.afiliado_id" class="text-red-600 text-sm mt-1">{{ form.errors.afiliado_id }}</div>
                        </div>

                        <fieldset :disabled="!form.afiliado_id" class="space-y-4 border-t dark:border-gray-700 pt-6">
                            ...
                            <div>
                                ...
                                <input id="codigo_medidor" v-model="form.codigo_medidor" type="text" class="mt-1 block w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 rounded-md shadow-sm" required />
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <p class="text-xs text-gray-500 mt-1">Autocompletado del afiliado (puede editarlo).</p>
                        ...
                        <div class="flex justify-end gap-4 mt-8 pt-6 border-t dark:border-gray-700">
                            <Link :href="route('conexiones.index')" class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded transition">Cancelar</Link>
        ...
        <Modal :show="showNuevaZonaModal" @close="showNuevaZonaModal = false" max-width="md">
             <div class="p-6 dark:bg-gray-800 dark:text-gray-200">
                ...
                <form @submit.prevent="guardarNuevaZona" class="mt-4 space-y-4">
                     <div>
                        <label for="nueva_zona_nombre_create" class="block font-medium text-sm ...">Nombre de la Zona *</label>
                        <input id="nueva_zona_nombre_create" v-model="nuevaZonaForm.nombre" type="text" class="mt-1 block w-full ..." required />
