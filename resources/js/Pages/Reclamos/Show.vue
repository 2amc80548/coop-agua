            ...
            <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
                ...
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg">
                    ...
                    <div class="p-6 md:p-8 border-b dark:border-gray-700">
                        <h3 class="text-lg font-semibold text-gray-800 dark:text-white mb-3">Asunto: {{ reclamo.asunto }}</h3>
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                            <p><strong class="text-gray-500 ...">Afiliado:</strong> {{ reclamo.afiliado?.nombre_completo }}</p>
                            <p><strong class="text-gray-500 ...">CI:</strong> {{ reclamo.afiliado?.ci }}</p>
                            <p><strong class="text-gray-500 ...">Tipo:</strong> {{ reclamo.tipo?.nombre }}</p>
                            <p><strong class="text-gray-500 ...">Medidor (si aplica):</strong> {{ reclamo.conexion?.codigo_medidor || 'N/A' }}</p>
                            <p><strong class="text-gray-500 ...">Estado:</strong> 
                                <span class="font-bold" :class="{
                                    'text-yellow-600': reclamo.estado === 'Abierto',
                                    'text-blue-600': reclamo.estado === 'En Revisión',
                                    'text-green-600': reclamo.estado === 'Resuelto',
                                    'text-gray-600': reclamo.estado === 'Cerrado',
                                }">{{ reclamo.estado }}</span>
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="p-6 md:p-8 space-y-6">
                        ...
                        <div v-if="isAdminOrSecretaria" class="border-t dark:border-gray-700 pt-6">
                            <h3 class="text-lg font-semibold text-gray-800 dark:text-white mb-3">Responder o Actualizar Estado</h3>
                            <form @submit.prevent="submitRespuesta" class="space-y-4">
                                <div>
                                    ...
                                    <textarea id="respuesta_admin" v-model="formRespuesta.respuesta_admin" rows="4" class="mt-1 block w-full ..." placeholder="Escriba la respuesta al afiliado..."></textarea>
                                    <div v-if="formRespuesta.errors.respuesta_admin" class="text-red-600 text-sm mt-1">{{ formRespuesta.errors.respuesta_admin }}</div>
                                </div>
                                <div>
                                    <label for="estado" class="block font-medium text-sm text-gray-700 dark:text-gray-300">Cambiar Estado</label>
                                    <select id="estado" v-model="formRespuesta.estado" class="mt-1 block w-full ...">
                                ...
                                <div class="flex justify-end">
                                    <button type="submit" :disabled="formRespuesta.processing" class="bg-blue-600 ...">
