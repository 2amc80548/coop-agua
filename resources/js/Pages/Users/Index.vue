    ...
    <table class="min-w-full bg-white border rounded">
      <thead class="bg-gray-50">
        <tr>
          ...
          <th class="py-3 px-4 text-left">Afiliado (CI)</th>
        ...
        <tr v-for="user in users" :key="user.id" class="border-t">
          ...
          <td class="py-3 px-4">
            <template v-if="user.afiliado">
              <div>{{ user.afiliado.nombre_completo }}</div>
              <div class="text-xs text-gray-500">CI: {{ user.afiliado.ci }}</div>
            </template>
            <span v-else class="text-gray-400">Sin afiliado</span>
          </td>
