...
<template>
  <AppLayout title="Lecturas">
    ...
    <div class="p-4 md:p-6">
      ...
      <Link
          v-if="$page.props.auth.user.role_names.includes('Administrador') || $page.props.auth.user.role_names.includes('Tecnico')"
      ...
      <div class="mb-4 p-4 bg-white dark:bg-gray-800 rounded shadow-sm grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
        <div class="sm:col-span-2">
          <label for="search" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Buscar (Medidor, CI/Nombre)</label>
        ...
        <div>
          <label for="periodo" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Período</label>
      ...
      <div class="overflow-x-auto bg-white dark:bg-gray-800 rounded shadow-md">
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          <thead class="bg-gray-50 dark:bg-gray-700">
            <tr>
              <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Medidor</th>
              <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Afiliado</th>
              ...
              <th scope="col" class="px-4 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">L. Ant.</th>
              <th scope="col" class="px-4 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">L. Act.</th>
              <th scope="col" class="px-4 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Consumo</th>
              ...
              <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Registró</th>
          ...
          <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
            <tr v-if="!lecturas.data.length">
              <td colspan="10" class="py-4 px-4 text-center text-sm text-gray-500 dark:text-gray-400">
            ...
            <tr v-for="l in lecturas.data" :key="l.id" class="hover:bg-gray-50 dark:hover:bg-gray-700 text-sm">
              <td class="px-4 py-2 whitespace-nowrap text-gray-700 dark:text-gray-300">{{ l.conexion?.codigo_medidor }}</td>
              <td class="px-4 py-2 whitespace-nowrap">
                  <div class="font-medium text-gray-900 dark:text-white">{{ l.conexion?.afiliado?.nombre_completo }}</div>
                  <div class="text-xs text-gray-500 dark:text-gray-400">CI: {{ l.conexion?.afiliado?.ci }}</div>
              </td>
              <td class="px-4 py-2 whitespace-nowrap text-gray-700 dark:text-gray-300">{{ l.periodo }}</td>
              <td class="px-4 py-2 whitespace-nowrap text-gray-700 dark:text-gray-300">{{ formatDate(l.fecha_lectura) }}</td>
              <td class="px-4 py-2 whitespace-nowrap text-right text-gray-700 dark:text-gray-300">{{ l.lectura_anterior }}</td>
              <td class="px-4 py-2 whitespace-nowrap text-right font-medium text-gray-900 dark:text-white">{{ l.lectura_actual }}</td>
              <td class="px-4 py-2 whitespace-nowrap text-right font-bold text-blue-700 dark:text-blue-400">
                  {{ (l.lectura_actual - l.lectura_anterior).toFixed(2) }} m³
              </td>
               <td class="px-4 py-2 whitespace-nowrap">
                  <span :class="{
                          'bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-100': l.estado === 'pendiente',
                          'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-100': l.estado === 'facturado',
                      }" class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full">
                      {{ l.estado }}
                  </span>
              </td>
              <td class="px-4 py-2 whitespace-nowrap text-xs text-gray-500 dark:text-gray-400">{{ l.usuarioRegistrado?.name }}</td>
              <td class="px-4 py-2 whitespace-nowrap text-sm font-medium space-x-2">
                <Link :href="route('lecturas.show', l.id)" class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-200" title="Ver Detalles">Ver</Link>
                <Link
                    v-if="l.estado === 'pendiente' && ($page.props.auth.user.role_names.includes('Administrador') || $page.props.auth.user.role_names.includes('Tecnico'))"
                    :href="route('lecturas.edit', l.id)"
                ...
                >
                 ...
                 <button @click="abrirAviso(l.id)" class="text-teal-600 hover:text-teal-900 dark:text-teal-400 dark:hover:text-teal-200" title="Imprimir Aviso">🖨️</button>
              </td>
            </tr>
          </tbody>
        </table>
