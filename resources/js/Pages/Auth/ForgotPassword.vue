...
<template>
    ...
    <div class="fixed inset-0 bg-gradient-to-br from-cyan-400 via-teal-500 to-blue-600 dark:bg-gray-900 dark:from-gray-900 dark:to-gray-800 overflow-hidden">
        <img src="/storage/img/2583.jpg" alt="Fondo" class="absolute inset-0 w-full h-full object-cover opacity-30 dark:opacity-5 dark:grayscale" onerror="this.style.display='none'" />
    </div>
    ...
    <div class="relative min-h-screen flex items-center justify-center p-4 md:p-8 z-10">
        <div class="bg-white/95 dark:bg-gray-800/95 backdrop-blur-sm rounded-3xl shadow-2xl p-6 sm:p-8 md:p-10 w-full max-w-xs sm:max-w-sm md:max-w-md animate-float-once">
            ...
            <h1 class="text-2xl sm:text-3xl md:text-4xl font-bold text-center text-teal-600 dark:text-cyan-400 mb-1 sm:mb-2">¿OLVIDASTE TU CONTRASEÑA?</h1>
            <p class="text-center text-gray-600 dark:text-gray-300 text-xs sm:text-sm md:text-base mb-6 sm:mb-8 px-2">Ingresa tu correo y te enviaremos un enlace para restablecerla.</p>

            <div v-if="status" class="mb-4 text-xs sm:text-sm font-medium text-green-600 dark:text-green-300 text-center bg-green-50 dark:bg-green-900 py-2 px-4 rounded-lg">
                {{ status }}
            </div>
            ...
            <div class="mt-6 text-center text-xs sm:text-sm text-gray-500 dark:text-gray-400">
                <Link :href="route('login')" class="text-teal-600 dark:text-cyan-400 hover:underline">Volver al inicio de sesión</Link>
            </div>
