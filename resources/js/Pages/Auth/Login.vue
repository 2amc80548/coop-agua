...
<template>
    ...
    <div class="relative min-h-screen flex items-center justify-center p-4 md:p-8 z-10">
        <div class="bg-white/95 backdrop-blur-sm rounded-3xl shadow-2xl p-6 sm:p-8 md:p-10 w-full max-w-xs sm:max-w-sm md:max-w-md animate-float-once">
            ...
            <h1 class="text-2xl sm:text-3xl md:text-4xl font-bold text-center text-teal-600 mb-1 sm:mb-2">AGUA CABEZAS</h1>
            <p class="text-center text-gray-600 text-xs sm:text-sm md:text-base mb-6 sm:mb-8 px-2">Asociación de Beneficiarios de Agua Potable</p>

            <div v-if="status" class="mb-4 sm:mb-6 text-xs sm:text-sm font-medium text-green-600 text-center bg-green-50 py-2 px-3 sm:px-4 rounded-lg">
                {{ status }}
            </div>
            ...
            <form @submit.prevent="submit" class="space-y-4 sm:space-y-6">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between text-xs sm:text-sm md:text-base space-y-3 sm:space-y-0">
                    ...
                    <Link v-if="canResetPassword" :href="route('password.request')" class="text-teal-600 hover:text-teal-700 hover:underline">
            ...
            <div class="mt-6 sm:mt-8 text-center text-xs sm:text-sm text-gray-500 space-y-1">
                <p>¿Primera vez? <Link :href="route('register')" class="text-teal-600 hover:underline">Regístrate aquí</Link></p>
