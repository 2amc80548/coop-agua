...
const buscar = () => {
    ...
    searchTimeout = setTimeout(async () => {
        ...
        try {
            // 1. Detectamos la carpeta del proyecto dinámicamente
            let baseUrl = route('dashboard'); 
            baseUrl = baseUrl.replace('/dashboard', ''); 

            const url = `${baseUrl}/buscar?q=${encodeURIComponent(q)}`;
        ...
        >
            ...
            <nav class="flex-1 overflow-y-auto p-4 md:p-5 space-y-3 md:space-y-4 scrollbar-thin scrollbar-thumb-cyan-400">
                <template v-if="$page.props.auth.user.role_names?.includes('Administrador')">
                        ...
                        <NavLink :href="route('users.index')" :active="route().current('users.*')" class="group flex items-center text-gray-700 dark:text-gray-300 hover:bg-cyan-50 dark:hover:bg-gray-700 px-3 md:px-4 py-3 md:py-3.5 rounded-xl transition-all duration-200 text-sm md:text-base">
                        ...
                        <NavLink :href="route('tarifas.index')" :active="route().current('tarifas.*')" class="group flex items-center text-gray-700 dark:text-gray-300 hover:bg-cyan-50 dark:hover:bg-gray-700 px-3 md:px-4 py-3 md:py-3.5 rounded-xl transition-all duration-200 text-sm md:text-base">
